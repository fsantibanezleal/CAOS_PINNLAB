...
def save_man(cid, m):
    (MANIFESTS / f"{cid}.json").write_text(
...
def bake_all():
    ...
    QUESTIONS["env-soil-heat-real"] = set_estimate(
        ...
        [
            item(
                "recovered diffusivity α",
                "difusividad recuperada α",
                value=f"{s.get('recovered_alpha_mm2_s', 0):.3f} mm²/s",
            ),
            item(
                "out-of-sample error (10/20/50 cm)",
                "error fuera de muestra (10/20/50 cm)",
                value=f"{s.get('holdout_rmse_c', 0):.2f} °C RMSE",
            ),
        ],
    )
    ...
    QUESTIONS["ind-heat2d-inverse"] = set_estimate(
        ...
        [
            item(
                "recovered conductivity map error",
                "error del mapa de conductividad",
                value=f"{cs.get('adapted_vs_std', 0) * 100:.1f}% vs k*",
            ),
            item(
                "without data (physics alone)",
                "sin datos (física sola)",
    ...
    mean = np.array(t["fields"]["c"])
    std = np.array(t["fields"]["c_std"])
    ix = int(np.argmin(np.abs(xs - 0.5)))
    it_ = int(np.argmin(np.abs(ts - 0.25)))
    ...
    QUESTIONS["poll-source-uq-bpinn"] = set_estimate(
        ...
        [
            ...
            item(
                "least-trusted spot (largest σ)",
                "punto menos confiable (mayor σ)",
    ...
    QUESTIONS["dyn-double-pendulum"] = set_estimate(
        ...
        [
            item(
                "PINN leave-time (|Δθ| > 0.3 rad vs RK45)",
                "leave-time de la PINN (|Δθ| > 0.3 rad vs RK45)",
                value=f"{s.get('leave_time', 0):.2f} s",
            ),
            item(
                "a 0.01 rad twin leaves the same tube at",
                "un gemelo a 0.01 rad deja el mismo tubo en",
    ...
    QUESTIONS["mine-thickener-settling"] = set_estimate(
        ...
        [
            item(
                "mudline passes z=0.5 at",
                "la línea de lodo cruza z=0.5 en",
                values=vals_settle,
                values_es=vals_settle_es,
            ),
    ...
    for kv in (float(ks[0]), float(ks[len(ks) // 2]), float(ks[-1])):
        ...
        items_flot.append(
            item(
                f"k = {kv:.2g}: recovery at t=1, time to 90%",
                f"k = {kv:.2g}: recuperación en t=1, tiempo a 90%",
                value=f"{R1:.1f}%, {t90s}",
                value_es=f"{R1:.1f}%, {t90s_es}",
            )
        )
    ...
    ts = np.array(fr["t"])
    cA = np.array(fr["frames"]["cA"])  # [frame][ix][iz]
    ...
    QUESTIONS["mine-heap-leach-rt"] = set_estimate(
        ...
        [
            item(
                "half-change breakthrough at the base",
                "ruptura de medio cambio en la base",
                value=f"t = {fmt(t_bt)}",
            ),
            item(
                "mean base cA shift over the window",
                "cambio medio de cA en la base",
                value=f"{base[0]:.2f} to {base[-1]:.2f}",
                value_es=f"{base[0]:.2f} a {base[-1]:.2f}",
            ),
        ],
    )
    ...
    for v in m["variants"]:
        ...
        below = ss <= s_star
        num = np.trapezoid(n[below, -1], ss[below])
        den = np.trapezoid(n[:, -1], ss)
    ...
    QUESTIONS["mine-comminution-pbe"] = set_estimate(
        ...
        [
            item(
                "passing fraction < 0.3 at t=1",
                "fracción pasante < 0.3 en t=1",
                values=vals_pass,
                values_es=vals_pass_es,
            )
        ],
    )
    ...
    ts = np.array(fr["t"])
    xs = np.array(fr["axes"]["x"])
    ys = np.array(fr["axes"]["y"])
    c = np.array(fr["frames"]["c"])  # [frame][ix][iy]
    xt, yt = 0.75, 0.65
    ix = int(np.argmin(np.abs(xs - xt)))
    iy = int(np.argmin(np.abs(ys - yt)))
    series = c[:, ix, iy]
    pk = float(series.max())
    tpk = float(ts[int(series.argmax())])
    ...
    QUESTIONS["poll-ocean-transport"] = set_estimate(
        ...
        [
            item(
                f"arrival (half-peak) at ({xt}, {yt})",
                f"llegada (medio pico) en ({xt}, {yt})",
                value=f"t = {fmt(t_arr)}",
            ),
            item(
                "peak concentration there (within t ≤ 1)",
                "concentración pico allí (dentro de t ≤ 1)",
                value=f"{pk:.3f} at t = {tpk:.2f}",
                value_es=f"{pk:.3f} en t = {tpk:.2f}",
            ),
        ],
    )
    ...
    QUESTIONS["poll-soil-barrier"] = set_estimate(
        ...
        [
            item(
                "half-rise time at x=0.75 (past the barrier)",
                "tiempo de media subida en x=0.75 (tras la barrera)",
                value=f"t = {fmt(t_half)}",
            ),
    ...
    for v in m["variants"]:
        t = trace_of(m, v["id"])
        zs = np.array(t["axes"]["z"])
        psi = np.array(t["fields"]["psi"])
    ...
    for v in m["variants"]:
        ...
        vals_th[v["id"]] = (
            f"t = {fmt(th)} (exact {exact:.3f})"
            if (th is not None and exact and exact <= ts[-1])
            else (f"t = {fmt(th)}" if th else "not in window")
        )
        vals_th_es[v["id"]] = (
            f"t = {fmt(th)} (exacto {exact:.3f})"
            if (th is not None and exact and exact <= ts[-1])
            else (f"t = {fmt(th)}" if th else "fuera de la ventana")
        )
    QUESTIONS["bench-heat1d"] = set_estimate(
        ...
        [
            item(
                "centerline half-cooling time",
                "tiempo de medio enfriamiento del centro",
                values=vals_th,
                values_es=vals_th_es,
            )
        ],
    )
    ...
    for v in m["variants"]:
        ...
        vals_arr_es[v["id"]] = (
            f"t = {fmt(t_arr)} (exacto 0.80)" if t_arr is not None else "fuera de la ventana"
        )
    QUESTIONS["bench-burgers1d"] = set_estimate(
        ...
        [
            item(
                "front (u=0.5) reaches x=0 at",
                "el frente (u=0.5) llega a x=0 en",
                values=vals_arr,
                values_es=vals_arr_es,
            )
        ],
    )
    ...
    t = trace_of(m)
    xs = np.array(t["axes"]["x"])
    u = np.array(t["fields"]["u"])
    col = u[:, -1]
    crossings = [
        float(xs[j] - col[j] * (xs[j + 1] - xs[j]) / ((col[j + 1] - col[j]) or 1))
    ...
    QUESTIONS["bench-allencahn"] = set_estimate(
        ...
        [
            item(
                "zero crossings of u(x, t=1)",
                "cruces por cero de u(x, t=1)",
                value=", ".join(f"{c:.3f}" for c in crossings) or "none",
            )
        ],
    )
    ...
    QUESTIONS["bench-navier-cavity"] = set_estimate(
        ...
        [
            item(
                "vortex core (stream-function extremum)",
                "núcleo del vórtice (extremo de la función corriente)",
                value=f"(x, y) = ({cx:.2f}, {cy:.2f})",
            ),
    ...
    for v in m["variants"]:
        ...
        vals_pk[v["id"]] = (
            f"|u|max = {abs(float(u[ij])):.4f} at ({float(xs[ij[0]]):.2f}, {float(ys[ij[1]]):.2f})"
        )
    ...
    for v in m["variants"]:
        ...
        vals_f[v["id"]] = (
            f"T = {fmt(meas)} (exact {2 / c:.2f})" if meas else f"T > window (exact {2 / c:.2f})"
        )
    QUESTIONS["bench-wave1d"] = set_estimate(
        ...
        [
            item(
                "measured period (4x first zero crossing)",
                "período medido (4x primer cruce por cero)",
                values=vals_f,
            )
        ],
    )
    ...
    u = np.array(t["fields"]["u"])
    px_, py_ = 0.25, 0.25
    iq = int(np.argmin(np.abs(xs - px_)))
    jq = int(np.argmin(np.abs(ys - py_)))
    val = float(u[iq, jq])
    exact = float(np.sin(6 * np.pi * px_) * np.sin(6 * np.pi * py_))
    QUESTIONS["ind-helmholtz"] = set_estimate(
        ...
        [
            item(
                "u at the receiver (Fourier PINN vs exact)",
                "u en el receptor (PINN Fourier vs exacta)",
                value=f"{val:.3f} vs {exact:.3f}",
            )
        ],
    )
    ...
    for v in m["variants"]:
        t = trace_of(m, v["id"])
        xs, ys = np.array(t["axes"]["x"]), np.array(t["axes"]["y"])
        up = np.array(t["fields"]["u_pred"])
        ut = np.array(t["fields"]["u_true"])
    ...
    QUESTIONS["bench-darcy-operator"] = set_estimate(
        ...
        [
            item(
                "peak head, FNO vs finite differences",
                "presión pico, FNO vs diferencias finitas",
                values=vals_pk,
            )
        ],
    )
    ...
    QUESTIONS["env-aquifer-test"] = set_estimate(
        ...
        [
            ...
            item(
                "recovery error (Cooper-Jacob)",
                "error de recuperación (Cooper-Jacob)",
                values=vals_err,
                values_es=vals_err_es,
            ),
        ],
    )
    ...
    for v in m["variants"]:
        ...
        vals_deg_es[v["id"]] = (
            "grilla de entrenamiento" if seen else f"{mm['degradation_x']}x vs 32x32, NO VISTA"
        )
    QUESTIONS["bench-darcy-superres"] = set_estimate(
        ...
        [
            ...
            item(
                "vs the 32x32 training grid",
                "frente a la grilla 32x32",
                values=vals_deg,
                values_es=vals_deg_es,
            ),
        ],
    )
    ...
    QUESTIONS["bench-darcy-conformal"] = set_estimate(
        ...
        [
            ...
            item(
                "coverage achieved on unseen fields",
                "cobertura lograda en campos no vistos",
                values=vals_emp,
                values_es=vals_emp_es,
            ),
        ],
    )
    ...
    QUESTIONS["dyn-pendulum-hnn"] = set_estimate(
        ...
        [
            item(
                "energy drift over 8 s",
                "deriva de energía en 8 s",
                values=vals_drift,
                values_es=vals_drift_es,
            ),
    ...
    vals_gain_es = {
        k: v.replace(" better", " mejor").replace(" WORSE", " PEOR") for k, v in vals_gain.items()
    }
    QUESTIONS["bench-darcy-pino"] = set_estimate(
        ...
        [
            item(
                "relative error, PINO (data + equation)",
                "error relativo, PINO (datos + ecuación)",
                values=vals_pino,
            ),
            item("relative error, data-only FNO", "error relativo, FNO solo-datos", values=vals_fno),
            item(
                "PINO vs the data-only operator",
                "PINO frente al operador solo-datos",
                values=vals_gain,
                values_es=vals_gain_es,
            ),
        ],
    )
    ...
    if (MANIFESTS / "ind-hidden-velocity.json").exists():
        ...
        QUESTIONS["ind-hidden-velocity"] = set_estimate(
            ...
            [
                item(
                    "recovered circulation center (true: 0.50, 0.50)",
                    "centro de circulación recuperado (verdadero: 0.50, 0.50)",
                    value=f"(x, y) = ({cx:.2f}, {cy:.2f})",
                ),
                item(
                    "current error INSIDE the dye-swept region",
                    "error de corriente DENTRO de la región barrida",
                    value=f"{s.get('speed_rel_rmse_swept', 0) * 100:.1f}% rel RMSE",
                ),
                item(
                    "in the never-dyed dead zones (unidentifiable)",
                    "en las zonas muertas sin tinte (no identificable)",
                    value=f"{s.get('speed_rel_rmse_dead', 0) * 100:.1f}%",
                ),
            ],
        )
    else:
...
def bake_markers():
    ...
    patch(
        "ind-hidden-velocity",
        markers=[
            _mk(float(g[0]), float(g[1]), "recovered center", "centro recuperado"),
    ...
    if th is not None:
        patch(
            "poll-soil-barrier",
            markers=[_mk(0.75, th, "half-rise past the wall", "media subida tras el muro")],
        )
    ...
    patch(
        "poll-source-uq-bpinn",
        markers=[
            _mk(0.5, 0.25, "checkpoint", "punto de control"),
            _mk(0.30, 0.0, "least trusted (max σ)", "menos confiable (σ máx)"),
        ],
    )
