...
def main():
    ...
    for case in registry.list_cases():
        ...
        truth = analytic(XY)
        if truth is None:
            skipped.append((cid, "analytic returned None"))
            continue
        truth = np.asarray(truth, dtype=float).reshape(shape)
        if truth.shape != adapted.shape:
            skipped.append((cid, f"shape mismatch {truth.shape} vs {adapted.shape}"))
            continue
        ...
        (DERIVED / cid / "comparison.json").write_text(
            json.dumps(cmp_trace, ensure_ascii=False), encoding="utf-8"
        )
