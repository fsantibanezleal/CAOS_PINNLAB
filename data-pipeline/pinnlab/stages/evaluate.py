...
dataset / FEM), plus the ONNX-vs-model parity (the train->web contract check). A case may additionally expose
`extra_metrics(sf)` for a bespoke anchor (e.g. the Ghia lid-driven-cavity centerlines). Leakage-safe: the reference
is the closed-form/dataset/FEM truth, never PINN output."""
...
def _eval_grid(case, mod):
    """The case's own evaluation grid if it defines one (datasets with a fixed grid), else the linspace grid."""
    if getattr(mod, "eval_grid", None) is not None:
        return mod.eval_grid()
    return linspace_grid(case.domain, case.grid)


def _truth_and_pred(case, mod, sf):
    ...
    if case.validation_anchor == "analytic" and getattr(mod, "analytic", None) is not None:
        _coords, XY, _shape = _eval_grid(case, mod)
...
def run(case_id: str, sf, onnx_info: dict) -> dict:
    ...
    truth, pred = _truth_and_pred(case, mod, sf)
