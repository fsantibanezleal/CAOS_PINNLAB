...
def main():
    ...
    for d in S._HOLDOUT:  # [10,20,50]
        ...
        meas_on_axis = np.interp(
            t_days_axis, days, meas_c
        )  # subsample the real series to the trace time axis
        ...
        lc.append(
            {
                ...
                "xLabel": "days from 2019-01-01",
                "yLabel": "T (degC)",
                "series": [
                    {
                        "label": f"measured {d} cm (real)",
                        "color": "var(--muted)",
                        "scatter": True,
                        "x": [round(float(x), 1) for x in t_days_axis],
                        "y": [round(float(y), 3) for y in meas_on_axis],
                    },
                    {
                        "label": "PINN (out-of-sample)",
                        "color": colors.get(d, "var(--good)"),
                        "x": [round(float(x), 1) for x in t_days_axis],
                        "y": [round(float(y), 3) for y in pinn_c],
                    },
                ],
            }
        )
    ...
    (DERIVED / "manifests" / f"{CID}.json").write_text(
