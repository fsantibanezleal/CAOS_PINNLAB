...
def main():
    ...
    print(
        f"[{cid}] naive_vs_std L2 = {rel * 100:.1f}%  (adapted = {tr['summary'].get('adapted_vs_std', '?')})",
        flush=True,
    )
    ...
    lanes = cmp["lanes"]
    if not any(lane["key"] == "naive" for lane in lanes):
        ins = next(
            (i for i, lane in enumerate(lanes) if lane["role"] == "reference"), 0
        ) + 1
        lanes.insert(
            ins,
            {
    ...
    cmp["note_en"] = (
        f"The naive plain PINN (no {case.method}) vs the {case.method} fix, both against the classical standard solution. Naive L2 = {rel * 100:.1f}%, adapted L2 = {a * 100:.1f}%."
        if a is not None
        else cmp.get("note_en", "")
    )
    cmp["note_es"] = (
        f"La PINN ingenua (sin {case.method}) frente a la corrección {case.method}, ambas contra la solución estándar clásica. L2 ingenua = {rel * 100:.1f}%, adaptada = {a * 100:.1f}%."
        if a is not None
        else cmp.get("note_es", "")
    )
