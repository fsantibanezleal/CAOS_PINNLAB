...
const METHODS: Method[] = [
  ...
  },
  {
    ...
    cases: "ind-heat2d-inverse, ind-hidden-velocity (HFM flagship), env-soil-heat-real (real data), poll-source-uq-bpinn (UQ)",
    ref: 9,
  },
];
...
/** The classical to SOTA to candidate-novel ladder per family (transcribed from the verified deep-research report,
...
const BEYOND: Record<string, { en: string; es: string; ref?: number }> = {
  ...
  "operator-learning": {
    ...
    es: "Frontera: el DeepONet físico aprende el operador solución (toda una familia a↦u), y la penalización física reduce su demanda de datos: un surrogate entrenable que responde una instancia nueva en una pasada [Wang 2021]. Candidato para PINN-Lab: un DeepONet físico sobre las familias paramétricas. Límite honesto: los operadores están acotados por la distribución: la precisión cae fuera de distribución, así que validar OOD contra una referencia clásica (como hace el caso Darcy).",
    ref: 18,
  },
};
    ...
    <section className="panel" style={{ marginBottom: 0, borderColor: "var(--accent-2)" }}>
        ...
        <p style={{ fontSize: 13.5 }}>
          {es
            ...
            : "The reason to use a PINN is not to solve the PDE: it is to estimate something that cannot be measured directly, using the PDE as the instrument. This is the published record (every claim verified against its primary source; see the reference beside each), and the catalogue case that teaches the same mechanism:"}
        </p>
