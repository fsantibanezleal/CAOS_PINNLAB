  ...
  const whatItIs = (
    <>
      <div className="panel" style={{ margin: "0 0 16px", borderColor: "var(--accent)" }}>
        ...
        <ul>
          <li>{es ? "¿Cuál es la difusividad térmica de ESTE suelo? No se puede medir in situ: dos series reales de temperatura de borde + la ecuación del calor la estiman (0.304 mm²/s), validada en profundidades que el optimizador nunca vio (~1 °C RMSE)." : "What is THIS soil's thermal diffusivity? You cannot measure it in situ: two real boundary temperature series + the heat equation estimate it (0.304 mm²/s), validated at depths the optimizer never saw (~1 degC RMSE)."}</li>
      ...
      <ul>
        ...
        <li>{es ? "No es un solver caja-negra. Todo es reproducible: el pipeline offline es determinista dado (caso, semilla), y el ONNX que corre tu navegador es exactamente la red validada (paridad < 1e-4)." : "Not a black-box solver. Everything is reproducible: the offline pipeline is deterministic given (case, seed), and the ONNX your browser runs is exactly the validated network (parity < 1e-4)."}</li>
      </ul>
      <p className="muted">{es ? "Sigue: Metodología (los métodos), Implementación (la arquitectura), Experimentos (qué se corrió), Benchmark (la honestidad)." : "Continue: Methodology (the methods), Implementation (the architecture), Experiments (what was run), Benchmark (the honesty)."}</p>
    </>
  );
  ...
  const winLose = (
    <>
      ...
      <ol>
        ...
        <li>{es ? "Directo difícil (helmholtz): la PINN ingenua se queda en ~121% por sesgo espectral (míralo NO aprender en Entrenamiento); las características de Fourier la llevan a 9%." : "Hard forward (helmholtz): the naive PINN sits at ~121% from spectral bias (watch it FAIL to learn in Training); Fourier features bring it to 9%."}</li>
