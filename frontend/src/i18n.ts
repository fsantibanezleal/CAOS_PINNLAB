...
const en = {
  ...
  footer: {
    ...
    complement: "a CAOS research lab",
    provenance: "Engine: DeepXDE (Lu et al. 2021, doi:10.1137/19M1274067, LGPL-2.1)",
    disclaimer: "Trains offline, runs the exported ONNX in the browser; synthetic-majority, honestly labeled.",
  ...
  arch: {
    ...
    tab: {
      ...
      bridge: "Train to ONNX to web",
      gate: "Live vs precompute",
      ladder: "Method ladder & dynamics",
      methods: "Methods & honesty",
    },
    overview: {
      text: "PINN-Lab is an offline pipeline plus a static, deterministic-replay web app (archetype ADR-0057). The pipeline is the product: it trains every physics-informed network offline, validates it, exports it to ONNX and bakes a compact artifact. The web app ships no training stack: it loads the baked artifacts and runs the exported network live in the browser. The map shows the full lifecycle: from a researched problem fiche, through the six-stage pipeline, to the committed artifacts and the deployed static site: what runs offline, what is precomputed, and what runs in the browser. On top of the base bake, dedicated ladder tools compute the standard-vs-naive-vs-adapted comparisons, the training-dynamics checkpoints and the 2-D evolution frames that power the Compare, Training and animated Field views (see the Method ladder & dynamics tab).",
    },
    web: {
...
const es: typeof en = {
  ...
  footer: {
    ...
    complement: "un laboratorio de investigación CAOS",
    provenance: "Motor: DeepXDE (Lu et al. 2021, doi:10.1137/19M1274067, LGPL-2.1)",
    disclaimer: "Entrena offline y ejecuta el ONNX exportado en el navegador; mayoría sintética, etiquetada con honestidad.",
    license: "MIT · uso investigativo y educativo",
    version: "Versión",
  },
  ...
  arch: {
    ...
    tab: {
      ...
      bridge: "Train a ONNX a web",
      gate: "Live vs precómputo",
      ladder: "Escalera de métodos y dinámica",
      methods: "Métodos y honestidad",
    },
    overview: {
      text: "PINN-Lab es un pipeline offline más una app web estática de replay determinista (arquetipo ADR-0057). El pipeline es el producto: entrena offline cada red informada por física, la valida, la exporta a ONNX y precalcula un artefacto compacto. La app web no incluye stack de entrenamiento: carga los artefactos precalculados y ejecuta la red exportada en vivo en el navegador. El mapa muestra todo el ciclo de vida: desde la ficha de problema investigada, por el pipeline de seis etapas, hasta los artefactos versionados y el sitio estático desplegado: qué se ejecuta offline, qué se precomputa y qué se ejecuta en el navegador. Sobre el precálculo base, herramientas dedicadas computan las comparaciones estándar-vs-ingenua-vs-adaptada, los checkpoints de dinámica de entrenamiento y los fotogramas de evolución 2-D que alimentan las vistas Comparar, Entrenamiento y Campo animado (ver la pestaña Escalera de métodos y dinámica).",
    },
    web: {
      text: "La app del navegador es una SPA estática Vite + React + TypeScript (HashRouter, estado con zustand, i18next EN/ES, ecuaciones KaTeX). Seleccionar un caso descarga su manifiesto + traza de campo precalculada y pinta un heatmap viridis con selector de salida y slider temporal; para los casos live, onnxruntime-web evalúa el .onnx exportado a cualquier resolución o sonda de cursor en tiempo real. Nada se computa en un servidor: la SPA se sirve como archivos estáticos en GitHub Pages, así el flujo web es: seleccionar, descargar manifiesto/traza/onnx, renderizar, sondear en vivo.",
    },
    pipeline: {
      text: "Offline, el motor `pinnlab` ejecuta seis etapas deterministas por caso: preprocess, feature_extraction, train, infer, evaluate, export: función pura de (caso, semilla). Dos contratos de datos versionados lo enmarcan: ingesta (tablas de observaciones para casos inversos, validadas contra esquema + una política explícita de outliers) y artefacto (el manifiesto + la traza de campo que lee la web). `train` es la etapa SOTA pesada (DeepXDE, Adam, luego L-BFGS, RAR opcional); `evaluate` es el test sin fuga contra un anclaje analítico / dataset / benchmark.",
    },
    bridge: {
      text: "Una PINN entrenada es cara de producir pero diminuta de evaluar: unas pocas capas densas. La etapa train exporta la red entrenada a ONNX (opset 18, exportador dynamo moderno, autocontenido) y verifica la paridad ONNX-vs-modelo < 1e-4, probando que el navegador evalúa exactamente la red validada: restricciones duras y features de Fourier precalculadas en el grafo. onnxruntime-web la ejecuta del lado cliente en bastante menos de un milisegundo. Este es el puente que convierte un método offline pesado en una página estática interactiva.",
    },
    gate: {
      text: "Cada caso se clasifica live o precompute a partir de mediciones, nunca a mano: live si y solo si el ONNX ≤ 4 MB y una inferencia de grilla completa ≤ 120 ms y la traza precalculada ≤ 1 MB. Un caso live envía su .onnx y se ejecuta interactivo en el navegador; un caso precompute envía solo la traza de replay. El veredicto y los tres números medidos van al manifiesto, y CI re-deriva el lane desde ellos para que una etiqueta errónea no pueda desplegarse: la frontera entre lo que se computa en vivo y lo precomputado es honesta y auditable.",
    },
    ladder: {
      text: "La tubería computa la escalera de métodos honesta por caso: la solución estándar (forma cerrada o solucionador clásico), la PINN ingenua (la red simple, entrenada de verdad para que su falla sea visible), la corrección adaptada y el carril con datos cuando el caso trata de datos; más los precalculados de dinámica: checkpoints de entrenamiento (la red aprendiendo, visible), fotogramas 2-D suaves, barridos y validaciones de benchmark. La web solo reproduce: Comparar (con la escalera animada), Entrenamiento, el héroe animado del Campo y Diagnóstico. Números reales: Helmholtz ingenua 120.8% vs Fourier 9.3%; Allen-Cahn ingenua 95.4% (colapso metaestable) vs restricciones duras+RAR 0.4%; heat2d-inverse sin datos 356% vs física+datos 4.0%. Donde una prueba justa no muestra contraste real, no se fabrica nada.",
    },
    methods: {
