...
const en = {
  ...
  footer: {
    ...
    disclaimer: "Trains offline, runs the exported ONNX in your browser; synthetic-majority, honestly labeled.",
...
const es: typeof en = {
  ...
  footer: {
    ...
    disclaimer: "Entrena offline y corre el ONNX exportado en tu navegador; mayoría sintética, etiquetada con honestidad.",
  ...
  arch: {
    ...
    overview: {
      text: "PINN-Lab es un pipeline offline más una app web estática de replay determinista (arquetipo ADR-0057). El pipeline ES el producto: entrena offline cada red informada por física, la valida, la exporta a ONNX y hornea un artefacto compacto. La app web no incluye stack de entrenamiento: carga los artefactos precalculados y ejecuta la red exportada en vivo en el navegador. El mapa muestra todo el ciclo de vida: desde la ficha de problema investigada, por el pipeline de seis etapas, hasta los artefactos versionados y el sitio estático desplegado: qué corre offline, qué se precomputa y qué corre en el navegador. Sobre el precálculo base, herramientas dedicadas computan las comparaciones estándar-vs-ingenua-vs-adaptada, los checkpoints de dinámica de entrenamiento y los fotogramas de evolución 2-D que alimentan las vistas Comparar, Entrenamiento y Campo animado (ver la pestaña Escalera de métodos y dinámica).",
    },
    web: {
    ...
    bridge: {
      text: "Una PINN entrenada es cara de producir pero diminuta de evaluar: unas pocas capas densas. La etapa train exporta la red entrenada a ONNX (opset 18, exportador dynamo moderno, autocontenido) y verifica la paridad ONNX-vs-modelo < 1e-4, probando que el navegador evalúa exactamente la red validada: restricciones duras y features de Fourier precalculadas en el grafo. onnxruntime-web la corre del lado cliente en bastante menos de un milisegundo. Este es el puente que convierte un método offline pesado en una página estática interactiva.",
    },
    gate: {
      text: "Cada caso se clasifica LIVE o PRECOMPUTE a partir de MEDICIONES, nunca a mano: live si y solo si el ONNX ≤ 4 MB Y una inferencia de grilla completa ≤ 120 ms Y la traza precalculada ≤ 1 MB. Un caso LIVE envía su .onnx y corre interactivo en el navegador; un caso PRECOMPUTE envía solo la traza de replay. El veredicto y los tres números medidos van al manifiesto, y CI re-deriva el lane desde ellos para que una etiqueta errónea no pueda desplegarse: la frontera entre lo que se computa en vivo y lo precomputado es honesta y auditable.",
    },
    ladder: {
      text: "La tubería computa la ESCALERA DE MÉTODOS honesta por caso: la solución estándar (forma cerrada o solucionador clásico), la PINN ingenua (la red simple, entrenada de verdad para que su falla sea visible), la corrección adaptada y el carril con datos cuando el caso trata de datos; más los precalculados de dinámica: checkpoints de entrenamiento (míralo aprender), fotogramas 2-D suaves, barridos y validaciones de benchmark. La web solo reproduce: Comparar (con la escalera animada), Entrenamiento, el héroe animado del Campo y Diagnóstico. Números reales: Helmholtz ingenua 120.8% vs Fourier 9.3%; Allen-Cahn ingenua 95.4% (colapso metaestable) vs restricciones duras+RAR 0.4%; heat2d-inverse sin datos 356% vs física+datos 4.0%. Donde una prueba justa no muestra contraste real, no se fabrica nada.",
    },
    methods: {
